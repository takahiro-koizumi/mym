<?php
/**
 * Template part for layout in single-news.php
 *
 * @since 1.0.0
...
$breadcrumb_position = get_theme_mod( 'post_news_breadcrumb_position', 'content_top' );
$hide_sidebar        = post_custom( 'emanon_hide_sidebar' );
if ( $hide_sidebar ) {
	$col               = 'one-col';
} else {
	$col               = get_theme_mod( 'post_news_layout','two-r-col' );
}
	...
	<div class="l-content">
		...
		<div class="l-content__inner">
			...
			<main class="l-content__main<?php echo esc_attr( $class_featured ); ?>">
				<?php get_template_part( 'template-parts/content/post/content-news' ); ?>
			</main>
			<?php
				if ( 'one-col' !== $col ) {
					get_sidebar();
