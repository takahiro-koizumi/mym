<?php
/**
 * Widget request section class
 *
 * @since 1.0.0
 * @package Emanon Premium
 */

class Emanon_Request_Section_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'request_section', // Base ID
			__( '[Es] 資料請求', 'emanon-premium' ), // Name
			array( 'description' => __( '資料請求一覧を表示するセクション。', 'emanon-premium' ), ) // Args
		);
	}
			...
			<div class="l-content" data-content-width="<?php echo esc_attr( $content_width ); ?>">
				<div class="u-row u-row-wrap wrapper-col has-sp-card-2<?php if ( 'sp-card-1' === $sp_col && $sp_slider ) { ?> u-post-scroll<?php } ?>">
					...
					<?php while ( $query->have_posts() ): $query->the_post(); ?>
						<?php
							$main_color   = get_theme_mod( 'main_color', '#007bbb' );
							$terms        = get_the_terms( get_the_ID(), 'request-cat' );
							$term_id      = '';
							$term_name    = '';
							...
							if ( $terms && ! is_wp_error( $terms ) ) {
								...
								$term_text_color                 = get_term_meta( $term_id, 'cat_text_color', '#ffffff' );
								if ( ! $term_text_color ) {
								$term_text_color       = '#ffffff';
								}
								$term_background_color           = get_term_meta( $term_id, 'cat_bg_color', $main_color );
								if ( ! $term_background_color ) {
								$term_background_color = $main_color;
								}
	...
	public function form( $instance ) {
		...
		<?php if ( ! get_posts( 'post_type=request' ) ): ?>
		<h4 class="red"><?php _e( '資料請求ページの設定が必要です。','emanon-premium' ); ?></h4>
		<?php endif; ?>
		...
		<p>
			...
			<?php
				$sp_col_type = [
					...
					'sp-card-3' => __( '［カード］3カラム', 'emanon-premium' ),
				];
			?>
...
} // class Request Section Widget

// Widgetの登録
add_action( 'widgets_init', 'emanon_request_section_widget' );
	function emanon_request_section_widget() {
		register_widget( 'Emanon_Request_Section_Widget' );
}