<?php
/**
 * Widget Search section class
 *
 * @since 1.0.0
 * @package Emanon Premium
 */

class Emanon_Search_Section_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'search_section', // Base ID
			__( '[Es] カスタム検索', 'emanon-premium' ), // Name
			array( 'description' => __( 'カスタム検索を表示するセクション。', 'emanon-premium' ), ) // Args
		);
	}
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<small class="notes"><span class="red">※</span><?php _e( 'Emanon設定＞タブ「検索」の設定が反映されます。','emanon-premium' ); ?></small>
		</p>
...
} // class Search_Section_Widget

// Widgetの登録
add_action( 'widgets_init', 'emanon_register_search_section_widget' );
	function emanon_register_search_section_widget() {
		register_widget( 'Emanon_Search_Section_Widget' );
}
