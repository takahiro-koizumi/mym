<?php
/**
 * Archive request header full width overlay
 *
 * @since 1.0.0
 * @package Emanon Premium
 */

$image_sp          = get_theme_mod( 'request_taxonomy_header_image_sp' );
if ( is_mobile() && $image_sp ) {
	$image           = get_theme_mod( 'request_taxonomy_header_image_sp' );
	$size            = '600_338';
} else {
	$image           = get_theme_mod( 'request_taxonomy_header_image_pc' );
	$size            = '1200_675';
}
$title             = get_theme_mod( 'request_taxonomy_title' );
$sub_title         = get_theme_mod( 'request_taxonomy_sub_title' );
$message           = get_theme_mod( 'request_taxonomy_message' );
