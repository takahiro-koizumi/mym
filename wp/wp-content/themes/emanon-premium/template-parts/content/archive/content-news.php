...
<?php
	if ( ! is_paged() && 'taxonomy_term_header_standard' === $header_style || ! is_paged() && 'taxonomy_term_header_full_width' === $header_style ) {
		get_template_part( 'template-parts/parts/featured-image/archive/news/news-header-standard' );
	} elseif ( ! is_paged() && 'taxonomy_term_header_center' === $header_style ) {
		get_template_part( 'template-parts/parts/featured-image/archive/news/news-header-center' );
	}
	if ( $share ) {
	...
	<?php
		if ( have_posts() ) :
			while ( have_posts() ) : the_post();
				get_template_part( 'template-parts/parts/archive-list/archive-list-news' );
			endwhile;
		endif;
	?>
</div>
