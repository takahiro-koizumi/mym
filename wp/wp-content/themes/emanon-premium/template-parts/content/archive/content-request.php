<?php
/**
 * Template part for displaying request archive
 *
 * @since 1.0.0
...
<?php
	if ( ! is_paged() && 'taxonomy_term_header_standard' === $header_style || ! is_paged() && 'taxonomy_term_header_full_width' === $header_style ) {
		get_template_part( 'template-parts/parts/featured-image/archive/request/request-header-standard' );
	} elseif ( ! is_paged() && 'taxonomy_term_header_center' === $header_style ) {
		get_template_part( 'template-parts/parts/featured-image/archive/request/request-header-center' );
	}
	if ( $share ) {
